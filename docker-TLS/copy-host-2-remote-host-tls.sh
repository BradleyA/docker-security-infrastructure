...
echo -e "\nUSAGE"
echo    "   ${0} [<REMOTE_HOST>]"
echo -e "   ${0}  <REMOTE_HOST> [<WORKING_DIRECTORY>]\n"
...
#	Backup ${REMOTE_HOST}:/etc/docker/certs.d
	...
	ssh -t ${USER}@${REMOTE_HOST} "sudo mkdir -p /etc/docker/certs.d/daemon ; cd /etc ; sudo tar -pcf /tmp/${REMOTE_HOST}-${FILE_DATE_STAMP}.tar ./docker/certs.d/daemon ; sudo chown ${USER}.${USER} /tmp/${REMOTE_HOST}-${FILE_DATE_STAMP}.tar ; chmod 0400 /tmp/${REMOTE_HOST}-${FILE_DATE_STAMP}.tar"
