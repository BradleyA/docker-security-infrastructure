...
if ! [[ "${NUMBER_DAYS}" =~ ^[0-9]+$ ]] ; then
  new_message "${LINENO}" "ERROR" "  <NUMBER_DAYS> is not an interger.  <NUMBER_DAYS> is set to '${NUMBER_DAYS}'" 1>&2
  display_usage
  exit 1
fi
...
if [[ ! -d "${HOME}" ]] ; then
  display_help | more
  new_message "${LINENO}" "ERROR" "  ${USER} does not have a home directory on this system or ${USER} home directory is not ${HOME}" 1>&2
  exit 1
fi
