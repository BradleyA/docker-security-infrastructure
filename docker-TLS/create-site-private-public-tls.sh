...
#		USERHOME - location of admin home directory, default is /home/
...
if [ -e ${USERHOME}${ADMTLSUSER}/.docker/docker-ca/.private/ca-priv-key.pem ] ; then
        echo -e "${0} ${LINENO} [ERROR]:	Site private key ${USERHOME}${ADMTLSUSER}/.docker/docker-ca/.private/ca-priv-key.pem\n\talready exists, renaming existing site private key"   1>&2
...
echo    "${0} ${LINENO} [INFO]:	Creating public key good for ${NUMBERDAYS} days in ${USERHOME}${ADMTLSUSER}/.docker/docker-ca"	1>&2
