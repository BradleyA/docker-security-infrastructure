...
echo    "Verify and correct file and directory permissions for ${USERHOME}${TLSUSER}/.ssh"
...
if [ $(stat -Lc %a ${USERHOME}${TLSUSER}/.ssh/id_rsa) != 600 ]; then
	...
	chmod 0600 ${USERHOME}${TLSUSER}/.ssh/id_rsa
fi
...
if [ $(stat -Lc %a ${USERHOME}${TLSUSER}/.ssh/authorized_keys) != 600 ]; then
	...
	chmod 600 ${USERHOME}${TLSUSER}/.ssh/authorized_keys
fi
...
if [ $(stat -Lc %a ${USERHOME}${TLSUSER}/.ssh/known_hosts) != 644 ]; then
	...
	chmod 644 ${USERHOME}${TLSUSER}/.ssh/known_hosts
fi
